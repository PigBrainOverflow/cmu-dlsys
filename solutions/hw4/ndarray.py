...
class NDArray:
    ...
    def broadcast_to(self, new_shape):
        ...
        for i in range(len(new_shape)):
            if self.shape[i] == 1:
                ...
                assert new_shape[i] == self.shape[i], "shape error"
    ...
    def __getitem__(self, idxs):
        ...
        return NDArray.make(tuple(new_shape), tuple(new_strides), device=self.device, handle=self._handle, offset=new_offset)
        ### END YOUR SOLUTION
