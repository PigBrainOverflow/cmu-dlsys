...
def softmax_loss(Z, y):
    ...
    total_loss = np.sum([(np.log(np.sum(np.exp(z), 0)) - z[label]) for z, label in zip(Z, y)], 0)
    # for z, label in zip(Z, y):
    #     loss = np.log(np.sum(np.exp(z), 0)) - z[label]
    #     total_loss += loss
...
def softmax_regression_epoch(X, y, theta, lr = 0.1, batch=100):
    ...
    for batch_i in range(n_batch):
        batch_start = batch_i * batch
        batch_end = batch_start + batch
        X_batch = X[batch_start : batch_end]
        y_batch = y[batch_start : batch_end]
        Z_batch = np.exp(X_batch @ theta)   # matmul
        norm_Z_batch = Z_batch / np.sum(Z_batch, axis=1, keepdims=True)

        for i, label in enumerate(y_batch): # Z - I_y
            norm_Z_batch[i, label] -= 1

        g_batch = np.transpose(X_batch) @ norm_Z_batch

        theta -= lr / batch * g_batch
...
def nn_epoch(X, y, W1, W2, lr = 0.1, batch=100):
    ...
    for i in range(n_batch):
        ...
        # compute Z
        Z_batch = relu(X_batch @ W1)

        # compute G_2
        G2_batch = np.exp(Z_batch @ W2)
        G2_batch = G2_batch / np.sum(G2_batch, axis=1, keepdims=True)
        for i, label in enumerate(y_batch):
            G2_batch[i, label] -= 1

        # compute G_1
        G1_batch = (Z_batch > 0) * (G2_batch @ np.transpose(W2))

        # update
        W1 -= lr / batch * np.transpose(X_batch) @ G1_batch
        W2 -= lr / batch * np.transpose(Z_batch) @ G2_batch
