...
class Tensor(Value):
    ...
    def __repr__(self):
        return "needle.Tensor(" + str(self.realize_cached_data()) + ", " + str(self.shape) + ")"
        ...
        if node.requires_grad:
            ...
            if node.op is not None:
                ...
                for input, partial_adjoint in zip(node.inputs, partial_adjoints):
                    # print(input.op, partial_adjoint)
...
def find_topo_sort(node_list: List[Value]) -> List[Value]:
    ...
    after all its predecessors are traversed due to post-order DFS, we get a topological sort.
    """
    ### BEGIN YOUR SOLUTION

    """
    res: List[Value] = []
    frontier: List[Value] = node_list.copy()    # a stack
    visited: Set[id(Value)] = set() # comparing id
    while len(frontier):
        cur = frontier.pop()
        if id(cur) not in visited:
            visited.add(id(cur))
            res.append(cur)
            for back in cur.inputs:
